#!/usr/bin/env python3
"""magi-llm-api — HTTP wrapper exposing melchior's Hailo-10H local LLM
...
with the live YOLO detection pipeline can strain melchior's current power
...
def chat():
    ...
    if not got_lock:
        return jsonify(
            error="busy",
            detail="melchior's Hailo-10H can only run one LLM session at a time "
